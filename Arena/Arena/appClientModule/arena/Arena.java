...
public class Arena extends JFrame implements KeyListener, MouseMotionListener, ActionListener{
	...
	JPanel logoAndScore = new JPanel();
    ...
    private JPanel createControlPanel() {
    	...
    	BoxLayout logoLayout = new BoxLayout(logoAndScore, BoxLayout.Y_AXIS);
    	logoAndScore.setLayout(logoLayout);
    	logoAndScore.add(logo);
    	HUD.add(healthBar);
    	scoreLbl.setBounds(new Rectangle(0,0,labelIcon.getIconWidth(), 20));
    	logoAndScore.add(scoreLbl);
    	controls.add(logoAndScore);
    	updateScore(score);
    	controls.add(HUD);
        return controls;
    }
	...
	{
		scoreLbl.setText("Press Ctrl to attack. Kills: " + Integer.toString(score));
		scoreLbl.invalidate();
	}
    ...
    public Vector<EnemyCharacter> getEnemyList()
    {
    	return enemies;
    }
}
